...
if [[ "$namespace" ]]; then
    ...
    # Rollout status
    microk8s kubectl -n $namespace rollout status deployments  flask-app-deployment|| {
        microk8s kubectl -n $namespace rollout undo deployments flask-app-deployment; exit 1;
    }
else
